...
<!-- White Paragraph  -->					
	...
	<h4 id="heading-1" class="ui-bar ui-bar-a ui-corner-all" align="left">User Login Information</h4>
	
		<div data-role="content"  class="ui-body ui-body-a ui-corner-all" >
		<p>Search can be used to filter user name or logged dates of user accounts.</p>
	<form>
...
<%--Using JSTL to draw user table from the database--%>        
            <c:forEach items="<%=dao.getAllLoginInfos()%>" var="logininfo">
				<tr>  
                    <td><c:out value="${logininfo.logID}" /></td>
                    <td><c:out value="${logininfo.userName}" /></td>
                    <td><c:out value="${logininfo.updateTime}" /></td>                
